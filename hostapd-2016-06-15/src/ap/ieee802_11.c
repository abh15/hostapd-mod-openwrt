...
//------------------------- Association Forwarding Code IITB ---------------------------------------------------

void start_client (int * sockfd, int status,char * server_port, char * server_ip)
{
		...
		if((status = getaddrinfo(server_ip, server_port, &hints, &servinfo))!=0)
			{
			fprintf(stderr, "error: %s while calling getaddrinfo\n", gai_strerror(status));
			exit(1);
			}
...
int send_msg(int*sockfd,char*buffer,int len)
{
	int status = 0;
	int left = len;//number of bytes left to be sent
	int sent = 0;//number of bytes sent so far
	while(sent<len)
	{
		status = send(*sockfd,buffer+sent,left,0);//send the number of bytes left, starting off from where the last byte was sent
		if (status == -1)
		{
			perror("failed send()");
			break;
	...
	if(status==-1) //Failed to send message
		return 1;
	else
		return 0;
}
/*
It's the responsibility of this function to receive 
len bytes of data via the recv() function
*/
int recv_msg( int *sockfd,char*buffer,int len)
{	
	int status =0;
	int received=0;//number of bytes received so far
	int left = len;//number of bytes left to be received
	while ( received<len ) 
	{
		status = recv( *sockfd, buffer+received,left, 0 );//receive the number of bytes left, starting off from where the last byte was received
	...
	if(status==-1) //Failed to recv message
		return 1;
	else
		return 0;
}

static int check_policy(struct sta_info *sta,char* AP_MAC)
{	
	int status=0;
	int sockfd;
...
static u16 check_assoc_ies(struct hostapd_data *hapd, struct sta_info *sta,
			   const u8 *ies, size_t ies_len, int reassoc,char * AP_MAC)
{
	struct ieee802_11_elems elems;
	u16 resp;
	const u8 *wpa_ie;
			 ...
			 int reassoc)
{
	...
	sta->capability = capab_info;
        ...
        char AP_MAC[18];
	sprintf(AP_MAC,"%02x:%02x:%02x:%02x:%02x:%02x",(mgmt->da)[0],(mgmt->da)[1],(mgmt->da)[2],(mgmt->da)[3],(mgmt->da)[4],(mgmt->da)[5]);

	/* followed by SSID and Supported rates; and HT capabilities if 802.11n
	 * is used */
	resp = check_assoc_ies(hapd, sta, pos, left, reassoc,AP_MAC);
	if (resp != WLAN_STATUS_SUCCESS)
		goto fail;
